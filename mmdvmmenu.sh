...
EOF
    ...
    "6")   sed -e 's/\/\/ #define WIDE_C4FSK_FILTERS_TX/#define WIDE_C4FSK_FILTERS_TX/' $conf > $conf.tmp && mv -f $conf.tmp $conf && echo "wider C4FSK filters for DMR and System Fusion on TX";;
    "7")   sed -e 's/\/\/ #define WIDE_C4FSK_FILTERS_RX/#define WIDE_C4FSK_FILTERS_RX/' $conf > $conf.tmp && mv -f $conf.tmp $conf && echo "wider C4FSK filters for DMR and System Fusion on RX";;
    "8")   sed -e 's/\/\/ #define SEND_RSSI_DATA/#define SEND_RSSI_DATA/'               $conf > $conf.tmp && mv -f $conf.tmp $conf && echo "RSSI Data Enabled";;
    "9")   sed -e 's/\/\/ #define ARDUINO_DUE_PAPA/#define  ARDUINO_DUE_PAPA/'          $conf > $conf.tmp && mv -f $conf.tmp $conf && echo "Layout for the PAPA board enabled";;
    "10")  sed -e 's/\/\/ #define ARDUINO_DUE_ZUM_V10/#define ARDUINO_DUE_ZUM_V10/'     $conf > $conf.tmp && mv -f $conf.tmp $conf && echo "Layout for ZUM board V1.0 enabled";;
    "11")  sed -e 's/\/\/ #define ARDUINO_DUE_NTH/#define ARDUINO_DUE_NTH/'             $conf > $conf.tmp && mv -f $conf.tmp $conf && echo "Layout for SP8NTH board enabled";;
    "12")  mv -f $confbak $conf ;;
